...
def validate_bigquery_access():
    ...
    try:
        ...
        if "billing" in str(e).lower():
            logger.error("This appears to be a billing-related issue. Please make sure billing is enabled for your project.")
            logger.error("Visit https://console.cloud.google.com/billing/linkedaccount?project=" + PROJECT_ID)
        return False

def main():
    """Main function to set up all GCP resources"""
    parser = argparse.ArgumentParser(description="Set up GCP resources for the travel data platform")
    parser.add_argument("--skip-bigquery", action="store_true", help="Skip BigQuery validation")
    parser.add_argument("--skip-gcs", action="store_true", help="Skip GCS bucket creation")
