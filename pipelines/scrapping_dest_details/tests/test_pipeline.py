...
from ..pipeline import (
    fetch_destinations_data,
    prepare_for_bigquery,
    run_pipeline
)
...
def test_fetch_destinations_data(mock_upload_raw, mock_get_dest_info, sample_destination_data, mock_raw_data):
    """Test fetching destination data from Wikipedia"""
    # Mock the destination list
    test_destinations = ['Paris', 'Rome', 'London']
    
    with patch('pipelines.scrapping_dest_details.pipeline.TRAVEL_DESTINATIONS', test_destinations):
...
def test_prepare_for_bigquery(sample_destination_data):
    ...
    expected_columns = [
        'destination_name', 'description', 'country', 'latitude', 'longitude',
        'population_count', 'population_year', 'timezone', 'languages',
        'climate', 'image_url', 'sections', 'area_km2', 'region',
        'attractions_count', 'attractions', 'ingestion_timestamp'
    ]
...
def test_run_pipeline_success(
    mock_bq_loader_class, mock_prepare, mock_upload_processed, mock_fetch,
    sample_destination_data, sample_processed_df, mock_bq_loader
):
...
def test_run_pipeline_bq_failure(
    mock_bq_loader_class, mock_prepare, mock_upload_processed, mock_fetch,
    sample_destination_data, sample_processed_df
):
