...
def fetch_destinations_data() -> List[Dict[str, Any]]:
    ...
    with ThreadPoolExecutor(max_workers=4) as executor:
        ...
        for future in as_completed(future_to_destination):
            ...
            try:
                ...
                if raw_data:
                    raw_data_upload_success = upload_raw_wiki_data(destination, raw_data)
    ...
    if failed_destinations:
        logger.warning(f"Failed to process {len(failed_destinations)} destinations: {', '.join(failed_destinations)}")
...
def prepare_for_bigquery(data: List[Dict[str, Any]]) -> pd.DataFrame:
    ...
    for item in data:
        ...
        record = {
            ...
            'attractions': json.dumps(item.get('attractions', [])),  # Store attractions as JSON array
            'ingestion_timestamp': item.get('ingestion_timestamp', datetime.datetime.now())
        }
